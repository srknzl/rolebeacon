...
`--start-ollama` is valid only when the saved scoring mode is Ollama. It never installs Ollama or pulls a
model. Fatal refresh or model-start failures still export the existing local database and return exit code 1;
